...
namespace B站视频历史评论删除工具
{
    ...
    internal class GetMd5
    {
        ...
        public static string RequestMd5(int flag, string oid, string webLocation, string next="")
        {
            string now = DateTimeOffset.UtcNow.ToUnixTimeSeconds().ToString();
            string md5code;
            if (flag == 0)
            {
                md5code = HttpUtility.UrlEncode($"mode=2&oid={oid}&pagination_str=%7B%22offset%22%3A%22%22%7D&plat=1&seek_rpid=&type=1&web_location={webLocation}&wts={now}{r}");
            }
            else
            {
                md5code = HttpUtility.UrlEncode($"mode=2&oid={oid}&pagination_str=%7B%22offset%22%3A%22%7B%5C%22type%5C%22%3A3%2C%5C%22direction%5C%22%3A1%2C%5C%22Data%5C%22%3A%7B%5C%22cursor%5C%22%3A{next}%7D%7D%22%7D&plat=1&type=1&web_location={webLocation}&wts={now}{r}");
            }
